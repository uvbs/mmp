 ...
 * @return {string} str
 */
function getMessage(msg, userUID) {
    var str = '',
    ...
    switch (msg.type) {
        ...
        case 'image':
            if (msg.status === -1) {
                ...
                str = '<a href="' + msg.file.url + '?imageView" target="_blank"><img onload="loadIMImg()" data-src="' + msg.file.url + '" src="' + msg.file.url + '?imageView&thumbnail=200x0&quality=85"/></a>';
            }
            break;
        case 'file':
            if (msg.status === -1) {
        ...
        case 'custom':
            ...
            if (content.type === 1) {
                ...
                str = '<img class="chartlet" onload="loadIMImg()" src="/lib/emoji/' + catalog + '/' + chartlet + '.png">';
 ...
 * @return {string} str
 */
function getSysFriendMessage(msg, user, userUID) {
    var msgHtml;
    msgHtml = ['<img class="img j-img" src="' + user.ico + '" data-account="' + user.to + '" title="' + user.toname + '" />',
                '<span class="nick">' + user.toname + '</span>'].join('');
    if (msg.type == 'applyFriend') {
        msgHtml = [
            msgHtml,
            '申请加您为好友！<br />',
            '<span class="j-pass" data-account="' + user.to + '"  data-nick="' + user.toname + '" data-ico="' + user.ico + '">同意</span>',
            '<span class="j-reject" data-account="' + user.to + '"  data-nick="' + user.toname + '">拒绝</span>'
        ].join('');
    }
    ...
    else if (msg.type == 'passFriendApply') {
        msgHtml = [
            msgHtml,
            '同意了您的好友申请！<br />'
        ].join('');
    }
    else if (msg.type == 'rejectFriendApply') {
        msgHtml = [
            msgHtml,
            '拒绝了您的好友申请！<br />'
        ].join('');
        ...
        msgHtml = [
            msgHtml,
            '已将您从好友中删除！<br />',
            '<span class="j-read">已读</span>'
        ].join('');
    }
    else {
        msgHtml = '收到一条[未知消息类型]消息';
    }
    return msgHtml;
}
function loadIMImg() {
    var scrollHeight = zcIM.IM.Form.find('.IM-chat-list').get(0).scrollHeight;
    var scrollTop = scrollHeight - zcIM.curScrollBottom;

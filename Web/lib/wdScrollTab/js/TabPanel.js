...
TabPanel.prototype = {
  ...
  refresh : function(position)
  {
    ...
    else
    {
      ...
      if(iframes.length>0)
      {
          ...
          var obj = window.frames[frameId];

          $(obj).attr('src', $(obj).attr('src'));

          //if (!obj) {
          //    obj = window.frames[this.tabs[position].id];
          //}
          //this.iterateFlush(obj);
      }
    }
  },
  ...
  show : function(position, notExecuteMoveSee){
    ...
    if(this.scrollFinish)
    {
      ...
      else
      {
        ...
        var preid = '';
        if (this.tabpanel_mover.find('.active').length > 0) {
            preid = this.tabpanel_mover.find('.active').get(0).id;
        }
        this.tabpanel_mover.find('.active').removeClass('active');
        this.tabs[position].preTabId = preid;
   ...
   * @param {String} id item id.
   */
  kill : function(position, toTabId){
    var tabEntity = this;
    //get tab index
    position = this.getTabPosision(position);
    ...
    /**
     * @description {Method} kill To close tab.
     * @param {String} id item id. List
     */
  killList: function (positionList, toTabId) {
      var tabEntity = this;
      var preTabId = null;
      for (var i = 0; i < positionList.length; i++) {
          var position = this.getTabPosision(positionList[i]);
          if (!preTabId) preTabId = this.tabs[position].preTabId;

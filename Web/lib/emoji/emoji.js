...
function buildEmoji(text) {
	...
	for (var j = 0, len = matches.length; j < len; ++j) {
		if(emoji[matches[j]]){
		    text = text.replace(matches[j], '<img class="emoji" src="http://static-files.socialcrmyun.com/lib/emoji/emoji/' + emoji[matches[j]].file + '" />');
		}		
	}
	return text;
}
...
CEmojiEngine.prototype.__renderChangeCol = function(){
	for(var i in this._emojiList){
		...
		for(var key in emojiList){
			...
			span.id = 'chn-emoji-'+i;	
			var img = new Image();
			img.src = "http://static-files.socialcrmyun.com/lib/emoji/" + i + "/" + emojiList[key]['file'];
	...
	for(var i in this._pinupList){
		...
		span.id = 'chn-pinup-'+i;
		var img = new Image();
		img.src = "http://static-files.socialcrmyun.com/lib/emoji/" + i + "/" + pinupList[0];
		span.appendChild(img);
		this._changeColumn.appendChild(span);		
	}
}
...
CEmojiEngine.prototype.__renderPictureCol= function(){
	...
	if(isEmoji){		//表情包
		...
		for(var key in emojiList){
			...
			var nw = this._lWidth < 34 ? 34 : this._lWidth;
			span.style.width = nw + 'px';

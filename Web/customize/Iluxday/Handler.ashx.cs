...
namespace ZentCloud.JubitIMP.Web.customize.Iluxday
{
    ...
    public class Handler : IHttpHandler, IReadOnlySessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                voteId = int.Parse(System.Configuration.ConfigurationManager.AppSettings["IluxdayVoteId"]);
                if (bllVote.IsLogin)
                {
        ...
        private string UpdateVoteObjectVoteCount(HttpContext context)
        {
            ...
            //检查是否可以点赞 每天1票
            int logCount = bllVote.GetCount<VoteLogInfo>(string.Format("VoteID={0} And UserID='{1}' And InsertDate>='{2}' And InsertDate<'{3}'", voteId, currentUserInfo.UserID, dateTimeToDay, dateTimeTomorrow));
            if (logCount >= 1)
            {
                resp.errcode = 1;
                resp.errmsg = "您今天已经点过赞了,明天再来吧";
                goto outoff;
            }
            //检查是否可以点赞 每天1票
        ...
        private string GetVoteObjectVoteList(HttpContext context)
        {
            ...
            List<VoteObjectInfo> sourceList = bllVote.GetVoteObjectInfoList(voteId, pageIndex, pageSize, out totalCount, keyWord, "", "1", sort);

    ...
    $.fn.zyComment = function (options, param) {
        ...
        return this.each(function () {
            ...
            this.createAgoCommentHtml = function () {
                ...
                html += '	<div class="text" style="display:none;font-size:2rem;padding-bottom:10px;border-bottom: 1px solid #DFDFDF;"> 大家的脚印 </div>';
                html += '</div>';
                $(self).append(html);
                ...
                $.each(para.agoComment, function (k, v) {
                    ...
                    item += '	<a class="avatar">';
                    item += '<div class="userimg" data-id="' + v.id + '">';
                    item += '		<img src="' + v.headimg + '">';
                    item += '</div>';
                    item += '	</a>';
            ...
            this.createFormCommentHtml = function () {
                ...
                // 组织发表评论的form html代码
                var boxHtml = '';
                boxHtml += '<form id="replyBoxAri" class="ui reply form" style="display:none;">';
            ...
            this.cancelReplyClickEvent = function () {
                ...
                $(self).find(".actions").on("click", '.cancel', function () {
                    // 1.移除之前的取消回复按钮
                    $(self).find(".cancel").remove();
            ...
            this.addReplyCommentFrom = function (id) {
                ...
                boxHtml += '<div class="form-group contentField field">';
                boxHtml += '<input  class="form-control" id="commentContent">';
                boxHtml += '</div>';
                boxHtml += '<div class="form-group textalign-right" id="publicComment">';
                boxHtml += '<a href="javascript:;" class="btn btn-default btn-reply">回复</a>';
                boxHtml += '</div>';

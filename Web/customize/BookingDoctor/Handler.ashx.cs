    ...
    public class Handler : ZentCloud.JubitIMP.Web.Serv.BaseHandler
    {
        ...
        private string GetDoctorList(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(categoryId))
            {
                ...
                productList = bllMall.GetList<WXMallProductInfo>(string.Format(" WebsiteOwner='{0}' And CategoryId in({1}) And ArticleCategoryType='BookingDoctor'  And Stock>0  And IsDelete=0", bllMall.WebsiteOwner, categoryIds));
            ...
            if (!string.IsNullOrEmpty(tags))
            {
                ...
                {
                    var data = bllMall.GetList<WXMallProductInfo>(string.Format(" WebsiteOwner='{0}'  And Stock>0 And ArticleCategoryType='BookingDoctor' And IsDelete=0 And Tags like '%{1}%'", bllMall.WebsiteOwner, tag));
        ...
        private string DoctorList(HttpContext context)
        {
            ...
            sbWhere.AppendFormat(" WebsiteOwner='{0}' And ArticleCategoryType='BookingDoctor' And IsDelete=0", bllMall.WebsiteOwner);

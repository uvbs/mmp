<%@ Page Title="" Language="C#" MasterPageFile="~/Master/WebMainContent.Master" AutoEventWireup="true" CodeBehind="Update.aspx.cs" Inherits="ZentCloud.JubitIMP.Web.Admin.Meifan.Activity.Update" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderWebMap" runat="server">
    <div class="title">
        当前位置：&nbsp;&nbsp;&nbsp;活动>&nbsp;&gt;&nbsp;&nbsp;<span>编辑</span> <a title="返回管理" style="float: right; margin-right: 20px;" href="List.aspx" class="easyui-linkbutton" iconcls="icon-back" plain="true">返回</a>
        ...
        <div style="font-size: 12px; width: 100%">
            <table width="100%;">
                <tr>
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtActivityName" maxlength="100" value="<%=activityInfo.ActivityName %>" style="width: 100%;" placeholder="名称(必填)" />
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtSummary" maxlength="100" value="<%=activityInfo.Summary %>" style="width: 100%;" placeholder="描述(选填)" />
                    </td>
                </tr>
                <tr style="display:none;">
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtMainPoints" maxlength="100" value="<%=activityInfo.MainPoints %>" style="width: 100%;" placeholder="要点(选填)" />
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td width="*" align="left">
                        <img alt="" src="<%=activityInfo.ThumbnailsPath %>" width="80px" id="imgActivity"  />
                        <br />
                 ...
                 <tr>
                    <td style="width: 100px;" align="right" class="tdTitle">开始时间：
                    </td>
                    <td width="*" align="left">
                        <input type="date" id="txtBeginDate"  value="<%=Convert.ToDateTime(activityInfo.BeginDate).ToString("yyyy-MM-dd") %>"   />
                    </td>
                </tr>
                  <tr>
                    ...
                    <td width="*" align="left">
                        <input type="date" id="txtEndDate"  value="<%=Convert.ToDateTime(activityInfo.EndDate).ToString("yyyy-MM-dd") %>" />
                    </td>
                </tr>
                  <tr style="display:none; ">
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtAddress"  value="<%=activityInfo.ActivityAddress %>" style="width: 100%;"  />
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td width="*" align="left">
                        <textarea id="txtDescription" style="width: 100%; height: 500px;"><%=activityInfo.ActivityDescription %></textarea>
                    </td>
                </tr>
            ...
            <strong style="font-size: 20px;">报名选项:</strong>
...
<asp:Content ID="Content4" ContentPlaceHolderID="bottom" runat="server">
    ...
    <script type="text/javascript">
        var handlerUrl = "/serv/api/admin/meifan/activity/update.ashx";
        var editor;
        var itemCount = 1; //数量
        var goupList = <%=GroupListJson%>;
        var memberTypeList = ["全部","会员", "非会员"];
        var items=<%=activityItemsJson%>;//选项
        ...
        $(function () {
            ...
            if (<%=activityInfo.IsFee%>==1) {
                ...
                $("#rdoIsNeedPay1").attr("checked","checked");
                $("#divItems").show();
            ...
            $("#btnAddItem").click(function () {
                ...
                //appendhtml.AppendFormat('开始日期:  <input type="datetime-local" class="begin-date" />');
                //appendhtml.AppendFormat('结束日期: <input type="datetime-local" class="end-date" />');
     ...
     'insertunorderedlist', '|', 'importword','video', 'image', 'multiimage', 'link', 'unlink','lineheight', '|', 'baidumap', '|', 'template', '|', 'table', 'cleardoc'],
        ...
        function getModel() {
            //模型
            var model = {
                activity_id:<%=activityInfo.JuActivityID%>,
        ...
        function loadItems() {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                appendhtml.AppendFormat('<div class="items" data-item-index="{0}" data-item-id="{1}">', i,items[i].AutoId);
                //appendhtml.AppendFormat('<img src="/img/icons/up.png" class="upfield fieldsort" />');
                //appendhtml.AppendFormat('<img src="/img/icons/down.png" class="downfield fieldsort" />');
                appendhtml.AppendFormat('<img src="/img/delete.png" class="delete-item" />');
                appendhtml.AppendFormat('<table style="width: 100%; margin-left: 10px;">');
                appendhtml.AppendFormat('<tr>');
                appendhtml.AppendFormat('<td>');
                //appendhtml.AppendFormat('开始日期:  <input type="datetime-local" class="begin-date" value="{0}"/>',items[i].FromDate);
                //appendhtml.AppendFormat('结束日期: <input type="datetime-local" class="end-date" value="{0}" />',items[i].ToDate);
                appendhtml.AppendFormat('组别:');
                appendhtml.AppendFormat('<select class="group-type">');
                //appendhtml.AppendFormat('<option value="无">无</option>');
                ...
                for (var l = 0; l < goupList.length; l++) {
                    if (items[i].GroupType == goupList[l]) {
                        appendhtml.AppendFormat('<option value="{0}" selected=\"selected\">{1}</option>', goupList[l], goupList[l]);
                    }
                    else {
                        appendhtml.AppendFormat('<option value="{0}">{1}</option>', goupList[l], goupList[l]);
                    }
                ...
                //appendhtml.AppendFormat('<option value="非会员">非会员</option>');
                for (var k = 0; k < memberTypeList.length; k++) {
                    if (items[i].IsMember == memberTypeList[k]) {
                        appendhtml.AppendFormat('<option value="{0}" selected=\"selected\">{1}</option>', memberTypeList[k], memberTypeList[k]);
                    }
                    else {
                        appendhtml.AppendFormat('<option value="{0}">{1}</option>', memberTypeList[k], memberTypeList[k]);
                    }
                ...
                appendhtml.AppendFormat('</select>');
                appendhtml.AppendFormat('金额:<input type="number" class="amount" value="{0}"/>',items[i].Amount);
                appendhtml.AppendFormat('</td>');
                appendhtml.AppendFormat('</tr>');
                appendhtml.AppendFormat('</table>');
                appendhtml.AppendFormat('</div>');

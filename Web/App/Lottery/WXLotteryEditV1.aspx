<%@ Page Title="" Language="C#" MasterPageFile="~/Master/WebMainContent.Master" AutoEventWireup="true"
    CodeBehind="WXLotteryEditV1.aspx.cs" Inherits="ZentCloud.JubitIMP.Web.App.Lottery.WXLotteryEditV1" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <link type="text/css" rel="Stylesheet" href="css/admincomm.css?v=20161014" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderWebMap" runat="server">
    当前位置：&nbsp;<a href="/App/Lottery/WXLotteryMgrV1.aspx">抽奖活动管理</a>&nbsp;&nbsp;&gt;&nbsp;&nbsp;<span>编辑
        <%=model.LotteryName %></span> <a href="<%=backUrl %>" style="float: right; margin-right: 20px; color: Black;"
            title="返回列表" class="easyui-linkbutton" iconcls="icon-back"
            plain="true">返回</a>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolderCnt" runat="server">
    <div class="ActivityBox">
        <div style="font-size: 12px; width: 100%">
            <table width="100%">
                <tr>
                    <td style="width: 100px;" align="right" class="tdTitle"><span class="colorRed">*</span>抽奖活动名称：
                    </td>
                    <td width="*" align="left">
                        <input type="text" id="txtLotteryName" value="<%=model.LotteryName %>" style="width: 100%;"
                            placeholder="抽奖活动名称(必填)" />
                    </td>
                </tr>
                <tr>
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtShareDesc" value="<%=model.ShareDesc%>" style="width: 100%;"
                            placeholder="将显示在微信分享描述中(选填)" />
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td width="*" align="left">
                        <img alt="" src="<%=model.ShareImg%>" width="100px;" id="imgshareimg" /><br />
               ...
               <tr class="wrapImgThumbnailsPath">
                    ...
                    <td width="*" align="left">
                        <img alt="" src="<%=model.ThumbnailsPath%>" width="30%" id="imgThumbnailsPath" /><br />
                 ...
                 <tr class="wrapBackGroundColor">
                    ...
                    <td width="*" align="left">
                        <input type="text" id="txtBackGroundColor" value="<%=model.BackGroundColor%>" style="width: 100px;"
                            placeholder="选填" />
                 ...
                 <tr>
                    <td style="width: 100px;" align="right" valign="top" class="tdTitle">抽奖方式：
                     ...
                     <td width="*" align="left">
                        ...
                        <input type="number" class="easyui-numberbox" style="width: 60px; margin-left: 12px;" value="<%=model.MaxCount%>"  id="txtMaxCount"/> 次
                    </td>
                </tr>
                <tr>
                    ...
                    <td width="*" align="left">
                        <br />
                        免费次数抽完之后，消耗 <input type="number" class="easyui-numberbox" style="width: 60px;" value="<%=model.UsePoints%>"  id="txtUsePoints" /> 积分可兑换一次抽奖机会( 0 或者空，则不允许积分兑换抽奖机会)
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
...
<asp:Content ID="Content4" ContentPlaceHolderID="bottom" runat="server">
    ...
    <script type="text/javascript">
        ...
        var selected="<%=model.ToolbarButton%>";
        var editor;
        var currLotteryID="<%=model.LotteryID%>";
        var lavm = new Vue({
        ...
        $(function () {
            ...
            InitAwardsData();
            ProcessPageElementShow(lotteryType,false);
            ...
            $('#btnSave').click(function () {
                try {
                    ...
                    $("input[name='lotteryproportion']").each(function () {
                        if ($.trim($(this).val())=="") {
                            checkresult=false;
                            $(this).focus();
                        }
                    ...
                    $.ajax({
                        type: 'post',
                        url: handlerUrl,
                        data: { Action: "EditWXLotteryV1", AutoID:<%=model.LotteryID%>,JsonData: JsonData },
                         dataType: "json",
                         success: function (resp) {
                             if (resp.Status == 1) {
                                 alert("更新成功");
             ...
             $("[data-action='addanswer']").live("click", function () {
                 $(this).closest("tr").before("<tr><td><input type=\"text\" name=\"lotteryname\" placeholder=\"奖品名称(必填)\" data-id=0 /></td><td><input type=\"text\" name=\"lotteryproportion\" placeholder=\"中奖概率(1-100)\" />%</td><td><input type=\"text\" name=\"lotterycount\" placeholder=\"奖品数量(必填)\" /></td><td><img src=\"/img/delete.png\" width=\"20\" height=\"20\"  class=\"deleteanswer\"/></td></tr>");

             });
             //添加答案选项

             $("input[name='lotteryproportion']").live("keyup", function () {

                 $(this).val($(this).val().replace(/\D/g, ''));

             })

             $("input[name='lotterycount']").live("keyup", function () {
             ...
             $('#txtStartTime').datetimebox('setValue', '<%=((DateTime)model.StartTime).ToString("yyyy-MM-dd HH:mm")%>');

            <%if (model.EndTime != null){ %>

                $('#txtEndTime').datetimebox('setValue', '<%=((DateTime)model.EndTime).ToString("yyyy-MM-dd HH:mm")%>');
            ...
            <%}%>
             ...
             if ("<%=model.LuckLimitType %>"=="1") {
                 rdoLuckLimitTypeDay.checked=true;
             }
             else {
                 rdoLuckLimitTypeAll.checked=true;
             }

             if ("<%=model.WinLimitType %>"=="1") {
                 rdowinLimitType1.checked=true;
             }
             else {
                 rdowinLimitType0.checked=true;
             }

             if ("<%=model.IsGetPrizeFromMobile %>"=="1") {
                 rdogetprizefrommobile.checked=true;
             }
             else if("<%=model.IsGetPrizeFromMobile %>"=="2") {
                rdogetprizefromsumbitinfo.checked=true;
            }
            else {
                rdogetprizefromback.checked=true;
            }
        ...
        function InitAwardsData(){
            $.getJSON(handlerUrl,{Action:'QueryAwardsSelect',LotteryID:currLotteryID},function(data){
                console.log("QueryAwardsSelect",data);
                if (data.IsSuccess) {
                    lavm.$data.awards = data.Result;
                }

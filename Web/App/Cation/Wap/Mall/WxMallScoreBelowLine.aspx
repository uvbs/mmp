<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WxMallScoreBelowLine.aspx.cs"
    Inherits="ZentCloud.JubitIMP.Web.App.Cation.Wap.Mall.WxMallScoreBelowLine" %>
...
<script type="text/javascript">
    ...
    function LoadScoreProductList(categoryid) {
        $.ajax({
            type: 'post',
            url: mallHandlerUrl,
            data: { Action: 'QueryScoreProductsObjList', Line: 1,Typeid:Typeid,PageIndex:1,PageSize:1000 },
            timeout: 60000,
            dataType:"json",
            success: function (resp) {
                ...
                for (var i = 0; i < resp.ExObj.length; i++) {
                    ...
                    str.AppendFormat('<a href="WxMallScoreBelowLineDetails.aspx?pid={0}" class="img">', resp.ExObj[i].AutoID);
                    str.AppendFormat('<img src="{0}">', resp.ExObj[i].RecommendImg);
                    str.AppendFormat('<h2>{0}</h2>', resp.ExObj[i].PName);
                    str.AppendFormat('</a>');
                    str.AppendFormat('<p class="price">所需积分：<span class="orangetext">{0}</span>积分</p>', resp.ExObj[i].Score);
                    str.AppendFormat('<a href="WxMallScoreBelowLineDetails.aspx?pid={0}" class="btn_min orange">我要兑换</a>', resp.ExObj[i].AutoID);

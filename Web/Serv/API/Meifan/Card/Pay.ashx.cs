...
namespace ZentCloud.JubitIMP.Web.Serv.API.Meifan.Card
{
    ...
    public class Pay : BaseHandlerNeedLoginNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string ip = "";//ip
            string notifyUrl = string.Format("http://{0}/WxPayNotify/NotifyOrderPay.aspx", context.Request.Url.Host);//支付通知地址
            string body = "";//订单内容
            ...
            var orderInfo = bllOrder.GetOrderPay(orderId);
            if (orderInfo != null)
            {
            ...
            try
            {
                string payReqStr = bllPay.GetBrandWcPayRequest(orderId, orderInfo.Total_Fee, appId, mchId, key, openId, ip, notifyUrl, body);

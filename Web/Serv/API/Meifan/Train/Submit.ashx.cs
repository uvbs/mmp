...
namespace ZentCloud.JubitIMP.Web.Serv.API.Meifan.Train
{
    ...
    /// 提交数据
    /// </summary>
    public class Submit : BaseHandlerNeedLoginNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (bll.GetCount<ActivityDataInfo>(string.Format(" ActivityId={0} And Name='{1}' And Phone='{2}' And DateRange='{3}' And GroupType='{4}' And IsMember='{5}'",requestModel.activity_id,requestModel.name,requestModel.phone,requestModel.date_range,requestModel.group_type,requestModel.member_type))>0)
            {
            ...
            model.ActivityType = "train";
            model.K1 = CurrentUserInfo.TrueName;
            model.K2 = CurrentUserInfo.Phone;
            if (bll.Add(model))
            {
                apiResp.status = true;
                apiResp.msg = "ok";

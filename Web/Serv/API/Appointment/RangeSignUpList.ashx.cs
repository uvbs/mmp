...
namespace ZentCloud.JubitIMP.Web.Serv.API.Appointment
{
    ...
    public class RangeSignUpList : BaseHandlerNeedLoginNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (sourceData.Where(p => p.Status >= 0).Count() > juInfo.LimitSignUpPassCount){
                isLimitPass = true;
            }
            foreach (var item in sourceData)
            {
                ...
                if (isLimitPass && item.Status == 0)
                {
                    requestModel.signup_status = -1;
                }
                else
                {
                ...
                requestModel.signup_phone = item.Phone;
                requestModel.signup_time = DateTimeHelper.DateTimeToUnixTimestamp(item.InsertDate);
                requestModel.signup_time_str = DateTimeHelper.DateTimeToString(item.InsertDate);
                ...
                if (userInfo != null)
                {
                    requestModel.signup_user_id = userInfo.AutoID;
                    requestModel.signup_user_name = userInfo.UserID;
                    ...
                    requestModel.signup_birthday_str = DateTimeHelper.DateTimeToString(userInfo.Birthday);
        ...
        public class RequestModel
        {
            ...
            /// 手机号
            /// </summary>
            public string signup_phone { get; set; }

            public int signup_user_id { get; set; }
            public string signup_user_name { get; set; }
            /// <summary>
            /// 报名时间
            /// </summary>

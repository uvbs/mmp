...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Activity.ActivityData
{
    ...
    public class Add : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int uId = 1001;
            ActivityDataInfo activity = bllActivity.Get<ActivityDataInfo>(string.Format(" ActivityID='{0}' AND WebsiteOwner='{1}' AND isDelete=0 Order by UID DESC",juActivity.SignUpActivityID,bllActivity.WebsiteOwner));
            if (activity != null)
            {
                uId = activity.UID + 1;
            }
            ...
            activityInfo.UID = uId;
            activityInfo.WebsiteOwner = bllActivity.WebsiteOwner;
            activityInfo.Name = requestModel.name;
            activityInfo.Phone = requestModel.phone;
            activityInfo.InsertDate = DateTime.Now;
            ...
            if (bllActivity.Add(activityInfo))
            {
                resp.errmsg = "ok";
                resp.isSuccess = true;
            }

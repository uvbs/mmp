...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Activity.Category
{
    ...
    public class Delete : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (bll.Delete(new ArticleCategory(), string.Format(" WebsiteOwner='{0}' AND CategoryType='activity' AND AutoID in ({1})", bll.WebsiteOwner, categoryIds)) == categoryIds.Split(',').Length)
            {
                resp.isSuccess = true;
                resp.errmsg = "ok";
                resp.errcode = 0;

...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Activity
{
    ...
    public class SetAccessLevel : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (blljuActivity.Update(new JuActivityInfo(), string.Format(" AccessLevel={0}", int.Parse(accessLevel)), string.Format(" WebsiteOwner='{0}' AND ArticleType='activity' AND JuActivityID in ({1})",blljuActivity.WebsiteOwner ,activityIds)) == activityIds.Split(',').Length)
            {
                resp.isSuccess = true;
                resp.errmsg = "ok";
            }

...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Mall.GroupBuyRule
{
    ...
    public class Update : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string msg = "";
            bool result = bllMall.UpdateProductGroupBuyRule(int.Parse(ruleId),ruleName, decimal.Parse(headDiscount), decimal.Parse(memberDiscount), int.Parse(peopleCount), int.Parse(expireDay), currentUserInfo.UserID, out msg);
            if (result)
            {

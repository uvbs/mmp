...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Article
{
    ...
    public class SetCategory : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (blljuActivity.Update(new JuActivityInfo(), string.Format(" CategoryId={0}", int.Parse(categoryId)), string.Format(" WebsiteOwner='{0}' AND ArticleType='article' AND JuActivityID in ({1})", blljuActivity.WebsiteOwner, activityIds)) == activityIds.Split(',').Length)
            {
                resp.errmsg = "ok";
                resp.isSuccess = true;
            }

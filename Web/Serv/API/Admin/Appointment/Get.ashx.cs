...
namespace ZentCloud.JubitIMP.Web.Serv.API.Admin.Appointment
{
    ...
    public class Get : BaseHandlerNeedLoginAdminNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (string.IsNullOrWhiteSpace(activity_id))
            {
            ...
            ActivityDataInfo actData = bll.GetActivityDataByUserId(list[0].SignUpActivityID, null,null
                , bll.WebsiteOwner);
            List<ActivityDataInfo> listSignupData = bll.GetActivityDataListByUId(list[0].SignUpActivityID, bll.WebsiteOwner, "1");
            apiResp.status = true;
            apiResp.code = 0;
            apiResp.result = new
            {

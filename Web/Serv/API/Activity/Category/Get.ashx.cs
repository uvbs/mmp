...
namespace ZentCloud.JubitIMP.Web.Serv.API.Activity.Category
{
    /// <summary>
    /// Get 的摘要说明
    /// </summary>
    public class Get : BaseHandlerNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ArticleCategory category = bll.Get<ArticleCategory>(string.Format(" WebsiteOwner='{0}' AND CategoryType='activity' AND AutoID={1}", bll.WebsiteOwner, categoryId));
            if (category == null)
            {
                resp.errmsg = "没有找到活动分类目录";
                resp.errcode = 1;
                context.Response.Write(ZentCloud.Common.JSONHelper.ObjectToJson(resp));
                return;
            }

    ...
    public class Update : BaseHanderOpen
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                var userInfo = bllUser.GetUserInfo(requestModel.user_id,bllUser.WebsiteOwner);
                if (userInfo!=null)
                {
                    action = "update";
                }
                ...
                if (string.IsNullOrEmpty(requestModel.password)&&action=="add")
                {
                ...
                {
                    string msg = "";
                    bool isSuccess = bllUser.UpdateSupplier(requestModel.store_id, requestModel.store_name, "", requestModel.store_tel, "", "", permissionGroupId, requestModel.store_image, requestModel.store_image, "", "", "", "", out msg, requestModel.address, requestModel.province, "", requestModel.city, "", requestModel.district, "", "", "");
                    ...
                    if (isSuccess)
                    {
                        ...
                        //更新门店表
                        JuActivityInfo pInfo = bllUser.Get<JuActivityInfo>(string.Format("WebsiteOwner='{0}' And ArticleType='Outlets' And K5='{1}'", bllUser.WebsiteOwner, requestModel.store_id));
                        // pInfo.JuActivityID = int.Parse(bllUser.GetGUID(BLLJIMP.TransacType.ActivityAdd));

...
namespace ZentCloud.JubitIMP.Web.Serv.API.ZentCloudOpen.Order
{
    ...
    public class List : BaseHanderOpen
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string lastUpdateFromTime = context.Request["lastupdate_from_time"];//订单最后更新时间开始
            string lastUpdateToTime = context.Request["lastupdate_to_time"];//订单最后更新时间结束

            int totalCount = 0;
            var orderList = bllMall.GetOrderList(pageSize, pageIndex, keyword, out totalCount, orderStatus, "", orderFromTime, orderToTime, orderType, giftOrderType,lastUpdateFromTime,lastUpdateToTime);
            ...
            foreach (var orderInfo in orderList)
            {
                ...
                model.delivery_time = (orderInfo.DeliveryTime!=null)?orderInfo.DeliveryTime.ToString():"";

...
{
    /// <summary>
    /// GetCompleteFieldLower 的摘要说明
    /// </summary>
    public class GetCompleteFieldLower : BaseHandlerNoAction
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (AvatarField != null)
            {
                resultList.Add(new GetCompleteField.ResultField{ field = "avatar", field_name = AvatarField.MappingName, type = AvatarField.FieldType, no_null = AvatarField.FieldIsNull, value = CurrentUserInfo.Avatar, read_only = AvatarField.IsReadOnly });
            }
            #endregion
            #region 姓名
            ...
            {
                resultList.Add(new GetCompleteField.ResultField{ field = "truename", field_name = TrueNameField.MappingName, type = "txt", no_null = TrueNameField.FieldIsNull, value = CurrentUserInfo.TrueName, read_only = TrueNameField.IsReadOnly });
            }
            #endregion
            #region 手机
            ...
            if (PhoneField == null)
            {
                ...
                resultList.Add(new GetCompleteField.ResultField{ field = "phone", field_name = "手机", type = "txt", no_null = 1, value = CurrentUserInfo.Phone, read_only = 0 });
            }
            else
            {
                resultList.Add(new GetCompleteField.ResultField{ field = "phone", field_name = PhoneField.MappingName, type = "txt", no_null = PhoneField.FieldIsNull, value = CurrentUserInfo.Phone, read_only = PhoneField.IsReadOnly });
            }
            #endregion
            ...
            foreach (var item in listFieldList.Where(p => !DefFields.Contains(p.Field)).OrderBy(p => p.Sort))
            {
                ...
                {
                    resultList.Add(new GetCompleteField.ResultField{ field = "describe", field_name = item.MappingName, type = FieldType, no_null = item.FieldIsNull, value = jtCurUser[item.Field].ToString(), read_only = item.IsReadOnly });
                }
                else
                {
                    resultList.Add(new GetCompleteField.ResultField{ field = item.Field.ToLower(), field_name = item.MappingName, type = FieldType, no_null = item.FieldIsNull, value = jtCurUser[item.Field].ToString(), read_only = item.IsReadOnly });
                }
            }
            apiResp.result = new
            {

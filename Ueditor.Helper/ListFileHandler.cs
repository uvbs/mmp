...
namespace Ueditor.Helper
{
    ...
    public class ListFileManager : Handler
    {
        ...
        public ListFileManager(HttpContext context, string pathToList, string[] searchExtensions, string configPath)
            : base(context)
        {
            this.SearchExtensions = searchExtensions.Select(x => x.ToLower()).ToArray();
            this.PathToList = pathToList;
        ...
        public override void Process()
        {
            try
            {
                ...
                Size = String.IsNullOrEmpty(Request["size"]) ? Config.GetInt("imageManagerListSize",this.ConfigPath) : Convert.ToInt32(Request["size"]);
            }

...
namespace Payment.Alipay
{
    ...
    public class Submit
    {
        ...
        /// <returns></returns>
        public static string BuildRequestRsaSign(string data, string key, string sign_type, string input_charset)
        {
            //把最终的字符串签名，获得签名结果
            string mysign = "";
            switch (sign_type)
            {
                case "MD5":
                    mysign = AlipayMD5.Sign(data, key, input_charset);
                    break;
                case "RSA":
                    mysign = RSAFromPkcs8.sign(data, key, input_charset);
                    break;
                case "0001":
                    mysign = RSAFromPkcs8.sign(data, key, input_charset);
                    break;
        ...
        /// <param name="sPara">请求给支付宝的参数数组</param>
        /// <returns>签名结果</returns>
        private static string BuildRequestMysignMall(Dictionary<string, string> sPara,string key)
        {
            //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
            string prestr = Core.CreateLinkString(sPara);
            ...
            {
                case "MD5":
                    mysign = AlipayMD5.Sign(prestr, key, _input_charset);
                    break;
                case "RSA":
                    mysign = RSAFromPkcs8.sign(prestr, _private_key, _input_charset);
                    break;
        ...
        private static Dictionary<string, string> BuildRequestPara(Dictionary<string, string> sParaTemp)
        {
            ...
            if (sPara["service"] != "alipay.wap.trade.create.direct" && sPara["service"] != "alipay.wap.auth.authAndExecute" && sPara["service"] != "refund_fastpay_by_platform_pwd")
            {
        ...
        /// <param name="sParaTemp">请求前的参数数组</param>
        /// <returns>要请求的参数数组</returns>
        private static Dictionary<string, string> BuildRequestParaMall(Dictionary<string, string> sParaTemp,string key)
        {
            //待签名请求参数数组
            Dictionary<string, string> sPara = new Dictionary<string, string>();
            //签名结果
            ...
            //获得签名结果
            mysign = BuildRequestMysignMall(sPara,key);

            //签名结果与签名方式加入请求提交参数组中
            sPara.Add("sign", mysign);
            if (sPara["service"] != "alipay.wap.trade.create.direct" && sPara["service"] != "alipay.wap.auth.authAndExecute" && sPara["service"] != "refund_fastpay_by_platform_nopwd")
            {
                sPara.Add("sign_type", _sign_type);
            }
        ...
        /// <param name="code">字符编码</param>
        /// <returns>要请求的参数数组字符串</returns>
        public static string BuildRequestParaToStringMall(Dictionary<string, string> sParaTemp, Encoding code,string key)
        {
            //待签名请求参数数组
            Dictionary<string, string> sPara = new Dictionary<string, string>();
            sPara = BuildRequestParaMall(sParaTemp,key);
            //把参数组中所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对参数值做urlencode
            string strRequestData = Core.CreateLinkStringUrlencode(sPara, code);
        ...
        public static string BuildRequest(string gateWay, Dictionary<string, string> sParaTemp, string strMethod, string strButtonValue)
        {
            //待请求参数数组
        ...
        /// <param name="strButtonValue">确认按钮显示文字</param>
        /// <returns>提交表单HTML文本</returns>
        public static string BuildRequestMall(string gateWay, Dictionary<string, string> sParaTemp, string strMethod, string strButtonValue,string key)
        {
            //待请求参数数组
            Dictionary<string, string> dicPara = new Dictionary<string, string>();
            dicPara = BuildRequestParaMall(sParaTemp,key);
            StringBuilder sbHtml = new StringBuilder();
        ...
        /// <param name="sParaTemp">请求参数数组</param>
        /// <returns>支付宝处理结果</returns>
        public static string BuildRequestMall(string gateWay, Dictionary<string, string> sParaTemp,string key)
        {
            Encoding code = Encoding.GetEncoding(_input_charset);

            //待请求参数数组字符串
            string strRequestData = BuildRequestParaToStringMall(sParaTemp, code,key);
        ...
        /// <returns>支付宝处理结果</returns>
        public static string BuildRequest(string gateWay, Dictionary<string, string> sParaTemp, string strMethod, string fileName, byte[] data, string contentType, int lengthFile)
        {
        ...
        public static string QueryTimestamp()
        {
            ...
            string url = gateWay + "service=query_timestamp&partner=" + Config.Partner + "&_input_charset=" + Config.Input_charset;
            string encrypt_key = "";

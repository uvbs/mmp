...
namespace Payment.Alipay
{
    ...
    public class Notify
    {
        ...
        /// <param name="sign">支付宝生成的签名结果</param>
        /// <returns>验证结果</returns>
        public bool VerifyReturnMall(Dictionary<string, string> inputPara, string sign,string key)
        {
            //获取返回时的签名验证结果
            bool isSign = GetSignVeryfyMall(inputPara, sign, true,key);
        ...
        /// <param name="sign">支付宝生成的签名结果</param>
        /// <returns>验证结果</returns>
        public bool VerifyNotifyMall(Dictionary<string, string> inputPara, string sign,string partner, string key)
        {
            //解密
            if (_sign_type == "0001")
            {
            ...
            try
            {
                ...
                if (notify_id != "") { responseTxt = GetResponseTxtMall(notify_id,partner); }
            }
            catch (Exception e)
            {
                responseTxt = e.ToString();
            ...
            //获取返回时的签名验证结果
            bool isSign = GetSignVeryfyMall(inputPara, sign, false,key);
        ...
        /// <param name="sign">支付宝生成的签名结果</param>
        /// <returns>验证结果</returns>
        public bool Verify(Dictionary<string, string> inputPara, string sign, string partner, string key)
        {
            //解密
            if (_sign_type == "0001")
            {
            ...
            {
                //XML解析notify_data数据，获取notify_id
                string notify_id = inputPara["notify_id"];
        ...
        /// <param name="isSort">是否对待签名数组排序</param>
        /// <returns>签名验证结果</returns>
        private bool GetSignVeryfyMall(Dictionary<string, string> inputPara, string sign, bool isSort,string key)
        {
            Dictionary<string, string> sPara = new Dictionary<string, string>();
            ...
            if (sign != null && sign != "")
            {
                ...
                {
                    case "MD5":
                        isSgin = AlipayMD5.Verify(preSignStr, sign, key, _input_charset);
                        break;
                    case "RSA":
                        isSgin = RSAFromPkcs8.verify(preSignStr, sign, _public_key, _input_charset);
                        break;
        ...
        /// <param name="notifyId">通知验证ID</param>
        /// <returns>验证结果</returns>
        private string GetResponseTxtMall(string notifyId,string partner)
        {
            string veryfyUrl = Https_veryfy_url + "partner=" + partner + "&notify_id=" + notifyId;

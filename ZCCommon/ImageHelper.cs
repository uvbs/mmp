...
namespace ZentCloud.Common
{
    public class ImageHelper
    {
        public class ImageClass
        {
            ...
            /// <param name="h"></param>
            /// <returns></returns>
            public static string ImageWatermark(string path, string waterpath,int x,int y,int w,int h,bool isCutEllipse = false)
            {
                string kz_name = Path.GetExtension(path);
                if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg")
                {
                    ...
                    if (isCutEllipse)
                    {
                        waterimg = CutEllipse(waterimg, new Rectangle(0, 0, waterimg.Width, waterimg.Height), new Size(waterimg.Width, waterimg.Height));
                    }

                    Graphics g = Graphics.FromImage(img);
                    g.DrawImage(waterimg, new Rectangle(x, y, w, w));
                    waterimg.Dispose();
                    g.Dispose();
                    string newpath = Path.GetDirectoryName(path) + filename + kz_name;
                    img.Save(newpath);
            ...
            /// <param name="location">水印位置</param>
            public static string LetterWatermark(string path, int size, string letter, Color color, Rectangle rgl, StringFormat sf)
            {
                #region
                ...
                if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg")
                {
                    ...
                    gs.DrawString(letter, font, br, rgl, sf);
                    gs.Dispose();
                    string newpath = Path.GetDirectoryName(path) + filename + kz_name;
                    img.Save(newpath);
                    img.Dispose();
            ...
            /// <param name="color">颜色</param>
            /// <param name="location">水印位置</param>
            public static string LetterWatermark(string path, Font font, string letter, Color color, Rectangle rgl, StringFormat sf)
            {
                #region
            ...
            public static void ZoomPicture(string sourceImage, string newImage,int targetWidth, int targetHeight)
            {
                var img = Image.FromFile(sourceImage);

                img = ZoomPicture(img, targetWidth, targetHeight);
